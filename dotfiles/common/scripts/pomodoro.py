...
#    color_stopped = '#2ECCFA'
    ...
    color_running = '#FFFFFF'
#    color_break = '#37FF00'
    color_break = '#FFFFFF'
    interval = 1
    short_break_count = 3
    format = '☯ {current_pomodoro}/{total_pomodoro} {time}'
    format_break = '☯ {current_pomodoro}/{total_pomodoro} {time}'
    ...
    def run(self):
        ...
        if self.state == RUNNING:
            ...
            sec = (self.time - datetime.utcnow()).total_seconds()
            #text = '{:02}:{:02}'.format(int(min), int(sec))
            text = '{:03}'.format(int(sec))
            sdict = {
                'time': text,
            ...
            if self.state == BREAK:
                ...
                color = self.color_break
                text = self.format_break.format(**sdict)
